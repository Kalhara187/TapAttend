...
export async function scanAttendance({ reqUser, employeeId, qrData }) {
  ...
  try {
    ...
    const [qrRows] = await connection.execute(
      `SELECT id, employee_id, email, qr_data, status, expires_at
       FROM qr_codes
       WHERE code = ?
       LIMIT 1
       FOR UPDATE`,
    ...
    const qrEmail = String(qrRecord.email || storedPayload?.email || '').trim().toLowerCase();
    const authEmail = String(reqUser.email || '').trim().toLowerCase();
    if (qrEmail && authEmail && qrEmail !== authEmail) {
      throw createError('This QR code does not belong to your account', 403);
    }
